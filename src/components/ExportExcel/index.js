...
function ExportExcel(column, dataSource, params) {
  // 新建工作谱
  const defaults = {
    fileName: '新建工作表',
    sheetName: 'sheet1'
  ...
  const file = new File();
  // 新建表
  let sheet = file.addSheet(params.sheetName);
  // 获取表头行数
  let depth = getDepth(column);
  // 获取表头的列数
  let columnNum = getColumns(column);
  // console.log(depth, columnNum)
  // 新建表头行数
  let rowArr = [];
  ...
  dataSource.map(ele => {
    ...
    columnLineArr.map(item => {
      dataTemp.push({
        [item.title]: ele[item.title],
        value: ele[item.title],
      });
    });
  ...
  file.saveAs('blob').then(function (content) {
    saveAs(content, params.fileName + '.xlsx');
  });
  ...
  function init(column, rowIndex, columnIndex) {
    column.map(item => {
      ...
      if (item.title === '操作') {
        ...
        //   });
        // }
        childrenNum = getColumns(item.children);
